...
### 2.5 Pre-release verification
Run a single command to make sure the TypeScript contracts and both builds pass before cutting a release candidate:

```bash
npm run verify
```

The script performs a root `tsc --noEmit`, the production Vite build, and the server TypeScript build (`npm run build --prefix server`).
