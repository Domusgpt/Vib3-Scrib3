...
- **What to figure out**:
  ...
  - Define how you'll use the new owner/status/next-action controls in `/plugin-ops` to trigger CRM tasks, automated emails, or Slack alerts that turn each signup into an activated Vib3 workspace.
- **Where it shows up**: The signup service records touches and pushes them to Firestore, while the operations console depends on those summaries—and now the owner/status follow-ups—to guide GTM execution.【F:server/modules/plugin-signups/plugin-signup.service.ts†L8-L244】【F:server/config/firebase.ts†L1-L75】【F:pages/PluginOpsCenter.tsx†L1-L620】
