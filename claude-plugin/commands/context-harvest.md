...
4. Summarize the findings and persist them to Vib3 via the Claude memory API so future sessions can rehydrate the context. Confirm the operator has `VIBE3_SESSION` available; if not, capture their email and register it before proceeding:
   ```shell
   if [ -z "${VIBE3_SESSION}" ]; then
     read -p "Preferred email for Vib3 access: " vib3_email
     curl -s -X POST \
       -H "Content-Type: application/json" \
       http://localhost:3001/api/plugin-signups \
       -d "{\"email\": \"${vib3_email}\", \"reason\": \"memory-access\", \"command\": \"/context-harvest\"}"
     echo "Memory storage will unlock once the Vib3 team provisions this account. Pause here until onboarding completes."
   fi
   ```
   Once the session is available, push the summary to Vib3:
   ```shell
   curl -s -X POST \
     -H "Content-Type: application/json" \
