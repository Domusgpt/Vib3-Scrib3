...
const DEFAULT_ALLOWED_PROVIDERS: SsoProvider[] = ['google', 'facebook'];
const INVITE_RATE_LIMIT_PER_HOUR = 12;
const INVITE_RATE_LIMIT_WINDOW_MS = 60 * 60 * 1000;
const INVITE_ANOMALY_THRESHOLD = Math.max(1, Math.floor(INVITE_RATE_LIMIT_PER_HOUR * 0.75));
...
class OrganizationService {
  async ensurePersonalWorkspace(user: User): Promise<Organization> {
    ...
    const organization: Organization = {
      ...
      authPolicy: this.createDefaultAuthPolicy(user.id, now),
    };
  ...
  async assertRole(
    ...
    this.enforceAuthPolicy(userId, organization);
  ...
  async createOrganization(userId: string, name: string, planId = 'scribe-pro'): Promise<OrganizationSummary> {
    ...
    const organization: Organization = {
      ...
      updatedAt: now,
      createdBy: userId,
      authPolicy: this.createDefaultAuthPolicy(userId, now),
    };
  ...
  async inviteMember(params: {
    ...
    const { organizationId, inviterId, email, role, expiresInDays = 14, overrideRateLimit = false } = params;
    ...
    const membership = await this.assertRole(inviterId, organizationId, ['owner', 'admin']);
    ...
    if (overrideRateLimit && membership.role !== 'owner') {
      throw new Error('Only workspace owners can override invitation limits.');
    }

    const actor = db.data.users.find(user => user.id === inviterId);

    const now = new Date();
    const windowStart = now.getTime() - INVITE_RATE_LIMIT_WINDOW_MS;
    const invitesInWindow = db.data.invitations.filter(invitation => {
    ...
    if (invitesInWindow >= INVITE_RATE_LIMIT_PER_HOUR) {
      if (!this.hasRecentAuditEvent(organizationId, 'organization.invite.rate_limited', INVITE_RATE_LIMIT_WINDOW_MS)) {
        await auditService.record({
          organizationId,
          actorId: inviterId,
          action: 'organization.invite.rate_limited',
          targetType: 'organization',
          metadata: {
            invitesInWindow,
            windowMinutes: Math.floor(INVITE_RATE_LIMIT_WINDOW_MS / (60 * 1000)),
            limit: INVITE_RATE_LIMIT_PER_HOUR,
          },
        });
      }
      await alertService.notifyGovernanceLimit({
        ...
        count: invitesInWindow,
        limit: INVITE_RATE_LIMIT_PER_HOUR,
        windowMinutes: Math.floor(INVITE_RATE_LIMIT_WINDOW_MS / (60 * 1000)),
        override: overrideRateLimit,
      });
      if (!overrideRateLimit) {
        throw new Error('Invite rate limit reached. Please wait before sending additional invitations.');
      }
      await auditService.record({
        organizationId,
        actorId: inviterId,
        action: 'organization.invite.rate_limit_overridden',
        targetType: 'organization',
        metadata: {
          invitesInWindow,
          limit: INVITE_RATE_LIMIT_PER_HOUR,
    ...
    if (
      invitesInWindow >= INVITE_ANOMALY_THRESHOLD &&
      !this.hasRecentAuditEvent(organizationId, 'organization.invite.anomaly_detected', INVITE_RATE_LIMIT_WINDOW_MS)
    ) {
      await auditService.record({
        organizationId,
        actorId: inviterId,
        action: 'organization.invite.anomaly_detected',
        targetType: 'organization',
        metadata: {
          invitesInWindow,
          limit: INVITE_RATE_LIMIT_PER_HOUR,
  ...
  async getAuthPolicy(organizationId: string): Promise<OrganizationAuthPolicy> {
    await db.read();
    if (!db.data) {
      throw new Error('Database not initialized');
    }
    ...
    return this.normalizeAuthPolicy(organization);
  }

  async updateAuthPolicy(
    actorId: string,
    organizationId: string,
    payload: OrganizationAuthPolicyUpdate,
    ...
    const sanitizedProviders = payload.allowedProviders.filter((provider): provider is SsoProvider =>
      DEFAULT_ALLOWED_PROVIDERS.includes(provider as SsoProvider),
    );

    if (payload.enforcement === 'required' && sanitizedProviders.length === 0) {
      throw new Error('Select at least one provider to enforce.');
    }

    const providers = sanitizedProviders.length > 0 ? Array.from(new Set(sanitizedProviders)) : [...DEFAULT_ALLOWED_PROVIDERS];
    const now = new Date().toISOString();
    const policy: OrganizationAuthPolicy = {
    ...
    organization.authPolicy = policy;
  ...
  private createDefaultAuthPolicy(actorId: string, timestamp: string): OrganizationAuthPolicy {
    return {
  ...
  private normalizeAuthPolicy(organization: Organization): OrganizationAuthPolicy {
    const source = organization.authPolicy;
    const allowed =
      source?.allowedProviders && source.allowedProviders.length > 0
        ? source.allowedProviders.filter((provider): provider is SsoProvider =>
            DEFAULT_ALLOWED_PROVIDERS.includes(provider as SsoProvider),
          )
        : [...DEFAULT_ALLOWED_PROVIDERS];

    return {
      enforcement: source?.enforcement ?? 'optional',
      allowedProviders: Array.from(new Set(allowed)),
      updatedAt: source?.updatedAt ?? organization.updatedAt,
      updatedBy: source?.updatedBy ?? organization.createdBy,
      note: source?.note,
    };
  }

  private enforceAuthPolicy(userId: string, organization: Organization): void {
    const policy = this.normalizeAuthPolicy(organization);
    ...
    const isCompliant = policy.allowedProviders.some(provider => {
      if (provider === 'google') {
        return Boolean(user.google);
      }
      if (provider === 'facebook') {
        return Boolean(user.facebook);
      }
      return false;
    });

    if (!isCompliant) {
      const providerLabels = policy.allowedProviders.map(provider => PROVIDER_LABELS[provider]).join(' or ');
      throw new Error(
        `This workspace requires signing in with ${providerLabels}. Please reconnect using an approved provider.`,
      );
    }
  }
