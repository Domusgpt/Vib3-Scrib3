...
const buildMemoryFirebasePayload = (memory: ClaudeMemory): Record<string, unknown> => {
  const payload: Record<string, unknown> = {
    ...
    sourceArtifacts: memory.sourceArtifacts.map(artifact => {
      const record: Record<string, unknown> = { label: artifact.label };
      if (artifact.url) {
        record.url = artifact.url;
      }
      if (artifact.excerpt) {
        record.excerpt = artifact.excerpt;
      }
      return record;
    }),
  ...
  if (memory.metadata && Object.keys(memory.metadata).length > 0) {
    payload.metadata = memory.metadata;
  }

  return payload;
};

const syncMemoryToFirebase = async (memory: ClaudeMemory) => {
  ...
  try {
    const collection = firestore.collection('users').doc(memory.userId).collection('claudeMemories');
    await collection.doc(memory.id).set(buildMemoryFirebasePayload(memory), { merge: true });
  } catch (error) {
    logger.error('Failed to sync Claude memory to Firebase.', { error });
  }
};
