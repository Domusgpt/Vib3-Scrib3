...
router.post('/:organizationId/api-keys', async (req: Request, res: Response) => {
  ...
  const overrideRateLimit = Boolean((req.body as { overrideRateLimit?: boolean }).overrideRateLimit);
  if (!req.user) {
    return res.status(401).json({ message: 'User not authenticated' });
  }
  ...
  try {
    const membership = await organizationService.assertRole(req.user.id, organizationId, ['owner', 'admin']);
    if (overrideRateLimit && membership.role !== 'owner') {
      return res.status(403).json({ message: 'Only workspace owners can override API key limits.' });
    }
