...
- (Optional) Add a root `.env` with `VITE_API_BASE_URL`, `VITE_RELEASE_CHANNEL`, and `VITE_DEV_API_URL` if the console will talk to a remote API origin or if your tooling needs explicit release metadata.
...
| Purpose | Command |
| --- | --- |
| Full release sweep (typecheck + tests + builds) | `npm run release:verify` |
| Type safety for the client | `npm run typecheck` |
| Server auth & license regression tests | `npm run test` |
...
## 4. Release Procedure
1. Run `npm run release:verify` to execute the consolidated release sweep. Resolve any failures before proceeding.
2. Ensure `npm run build` has produced the client assets in `dist/`.
3. Run `npm run build --prefix server` to emit compiled server output into `server/dist` (redundant if step 1 just ran, but safe when using CI artifacts).
4. Deploy the contents of `dist/` (client) and `server/dist` (Node service) to the target environment.
5. Start the API with `npm run start --prefix server` (or the equivalent process manager command in production).
6. Verify the deployment by calling `https://<host>/auth/user` (should return the session snapshot or `{ isAuthenticated: false }`) and then smoke test the console via the public URL.
7. Enable monitoring for the `alerts` module destinations (Slack + PagerDuty) before announcing the release.
