...
- **Summary tiles** surface total captured emails, the open queue (new + contacted), multi-touch accounts, and the most recent touch timestamp so you know whether onboarding requests are accelerating.
- **Reason and status breakdowns** show how often operators ask for memory access, profile sync, or advanced tools and how many are still in "new/contacted" versus "activated/closed" states.
- **Signup journal** lists the raw touches (reason, captured command, notes) with filters for time window, reason, and status. It relies on the authenticated `GET /api/plugin-signups` endpoint you just added, so only trusted operators can view the data.
