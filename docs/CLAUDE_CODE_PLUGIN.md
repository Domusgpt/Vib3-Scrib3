...
If you prefer to lean on Firebase instead of LowDB for plugin signups and memory storage (so you can reuse Firebase Auth and security rules), add the following values to `server/.env`:

```env
FIREBASE_PROJECT_ID=<your_project_id>
FIREBASE_CLIENT_EMAIL=<service_account_email>
FIREBASE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n"
# Optional collection overrides (defaults shown)
FIREBASE_MEMORIES_COLLECTION=claudeMemories
FIREBASE_SIGNUPS_COLLECTION=pluginSignups
```

When these variables are present the API will read/write Claude memories and plugin signup touches via Firestore. Otherwise it falls back to the bundled LowDB JSON store.
