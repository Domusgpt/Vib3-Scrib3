...
Run this quick battery before sharing the marketplace link so stakeholders can see the end-to-end workflow in action:

1. **Compile the platform**
   - `npm run typecheck`
   - `npm run build`
   - `npm run build --prefix server`
   Successful output from all three commands confirms the client bundle, API server, and shared types are ready to ship.
2. **Verify API coverage**
   - `curl -I http://localhost:3001/api/memory/primer` (should return `200 OK` when a session cookie is present)
   - `curl -s -X POST http://localhost:3001/api/plugin-signups -d '{"email":"qa@example.com","reason":"memory-access"}' -H 'Content-Type: application/json'`
   The signup request should respond with `202 Accepted`. Repeat with and without a session cookie to ensure onboarding gates work correctly.
3. **Confirm Firebase mirroring (if enabled)**
   - Start the API with `FIREBASE_EMULATOR_HOST` or production credentials.
   - Trigger `/context-harvest` and `/style-sync`, then check the Firestore console or emulator logs for new documents under `users/{userId}/claudeMemories` and `pluginSignups/`.
   - Review the server logs (`Firebase sync succeeded` entries) to confirm failures are surfaced without breaking the request.
4. **Exercise the Claude workflow**
   - Launch Claude Code, reinstall the marketplace, and run `/context-harvest → /style-sync → /memory-primer` in order.
   - Attempt to run a command while signed out of Vib3; verify the plugin pauses and records a signup entry instead of leaking context.
   - Run `/release-pulse` with fresh memories to double-check the primer summary references the latest highlights.

If all four buckets succeed, the plugin is ready for internal QA and marketplace publishing.
