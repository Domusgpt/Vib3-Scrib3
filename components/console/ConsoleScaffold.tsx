...
import ConsoleBootState from './ConsoleBootState';
import ConsoleErrorState from './ConsoleErrorState';
...
const ConsoleScaffold: React.FC<ConsoleScaffoldProps> = ({
    ...
    const guardableHandlers = React.useMemo(
        () => ({
            selectProfile,
            startTrial,
            upgradePlan,
        ...
        [openPortal, selectProfile, startTrial, switchOrganization, upgradePlan],
    );

    const {
        selectProfile: guardedProfileSelect,
        startTrial: guardedStartTrial,
        upgradePlan: guardedUpgradePlan,
        openPortal: guardedOpenPortal,
        switchOrganization: guardedSwitchOrganization,
    ...
    const resolvedContent = React.useMemo(() => {
        if (isError) {
            ...
            return (
                <div className="flex min-h-[360px] w-full justify-center px-6 py-16 sm:px-10">
                    <ConsoleErrorState
                        eyebrow={errorEyebrow ?? 'Workspace unavailable'}
                        title={errorTitle ?? 'We hit a snag'}
                        message={errorMessage ?? 'We couldn\'t load your workspace context. Please try again.'}
                        hint={errorHint}
        ...
        return (
            <div className="flex min-h-[360px] w-full justify-center px-6 py-16 sm:px-10">
                <ConsoleBootState
                    eyebrow={loadingEyebrow ?? 'Preparing console'}
                    message={loadingMessage ?? 'Loading workspace context…'}
                    hint={loadingHint}
                />
            </div>
        );
    ...
    return (
        <>
            ...
            >
                {banner}
                <div aria-busy={isLoading}>{resolvedContent}</div>
            </AppShell>
