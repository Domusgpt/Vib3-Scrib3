...
const WorkspaceSettings: React.FC = () => {
    ...
    const guardWithAuth = useCallback(
        <Args extends unknown[], ReturnType>(action: (...args: Args) => ReturnType) =>
            requireAuthentication(action, { openModal: true }),
        [requireAuthentication],
    );
    ...
    const handleSwitchOrganization = guardWithAuth(async (organizationId: string) => {
        setError(null);
    ...
    const handleInvite = guardWithAuth(async () => {
    ...
    const handleCreateApiKey = guardWithAuth(async () => {
    ...
    const handleCreateWebhook = guardWithAuth(async () => {
    ...
    const handleSaveAuthPolicy = guardWithAuth(async (payload: OrganizationAuthPolicyUpdate) => {
    ...
    const handleCreateWorkspace = guardWithAuth(async () => {
    ...
    const handleRevokeInvitation = guardWithAuth(async (invitationId: string) => {
        if (!selectedOrganization) return;
    ...
    const handleRevokeApiKey = guardWithAuth(async (keyId: string) => {
        if (!selectedOrganization) return;
    ...
    const handleExportAuditLog = guardWithAuth(async () => {
        if (!selectedOrganization) return;
    ...
    const handleDeleteWebhook = guardWithAuth(async (webhookId: string) => {
        if (!selectedOrganization) return;
    ...
    const handleTestWebhook = guardWithAuth(async (webhookId: string) => {
        if (!selectedOrganization) return;
        setError(null);
        setNotification(null);
    ...
    const handleChangeMemberRole = guardWithAuth(async (memberId: string, role: OrganizationRole) => {
        if (!selectedOrganization) return;
        setError(null);
        setNotification(null);
    ...
    const handleRemoveMember = guardWithAuth(async (memberId: string) => {
        if (!selectedOrganization) return;
    ...
    const handleStartTrial = guardWithAuth(async (planId: string) => {
        if (!selectedOrganization) return;
        setError(null);
        setNotification(null);
    ...
    const handleUpgrade = guardWithAuth(async (planId: string, cadence: 'monthly' | 'yearly') => {
        if (!selectedOrganization) return;
        setError(null);
        setNotification(null);
    ...
    const handleOpenPortal = guardWithAuth(async () => {
        if (!selectedOrganization) return;
        setError(null);
        setNotification(null);
